...
const char devDescriptor[] =
{
  ...
  0x10,   // bcdUSB L // USB 1.1
  0x01,   // bcdUSB H
//  0x00,   // bcdUSB L // USB 2.0
//  0x02,   // bcdUSB H 
      ...
      else if (wValue>>8 == STD_GET_DESCRIPTOR_STRING)
      {
        switch ( wValue & 0xff )
        {
          ...
          case STRING_INDEX_MANUFACTURER:
            USB_SendString(pCdc->pUsb, STRING_MANUFACTURER, STRING_MANUFACTURER_LENGTH, wLength );
          break;

          case STRING_INDEX_PRODUCT:
            USB_SendString(pCdc->pUsb, STRING_PRODUCT, STRING_PRODUCT_LENGTH, wLength );
          break;
          default:
            /* Stall the request */
...
uint32_t USB_SendString(Usb *pUsb, const char* ascii_string, uint8_t length, uint8_t maxLength)
{
  ...
  string_descriptor[0] = (length<<1) + 2;
